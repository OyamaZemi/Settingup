...
詳細は[Mac の初期設定](mac-setup.md)を参照。
...
新しい作業をするときは，`main` (旧 `master` ) に直接 commit せず，新しい branch を作るのが基本である．
...
共同作業では，直接 `main` (旧 `master` ) に push するのではなく，branch を作って commit し，Pull Request を出すことが多い．

基本的な流れは次の通りである．

1. 最新の `main` から新しい branch を作る
2. 変更する
3. stage する
4. commit する
5. GitHub に push する
